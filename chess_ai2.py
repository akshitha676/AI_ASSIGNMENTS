...
class State:
    ...
    def evaluate(self):
        ...
        (a) MATERIAL (pieces):
            Count pawns, rooks, knights, bishops, queens.
            Add their standard values to the score.
            Positive for White, negative for Black.

        (b) CENTER CONTROL:
            The 4 central squares are: d4, e4, d5, e5.
            A piece standing in the center increases the score.

        (c) MOBILITY:
            Number of legal moves available for White vs Black.
            More mobility = better position.

        (d) KING SAFETY (optional but fun):
            If a king is attacked, reduce its score.
        ...
        piece_values = {
            ...
            chess.ROOK: 5,
            chess.QUEEN: 9,
            chess.KING: 0  # King has no material value (game ends if lost)
        }
        ...
        if black_king is not None:
            ...
            score += 0.5 * len(attackers)  # Black king under attack
...
def play_game():
    ...
    maxDepth = 3  # Search depth (try changing this to make AI stronger/weaker)
